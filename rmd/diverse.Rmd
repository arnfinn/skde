...
- Legg inn stasjon i Feltet *Mappe* (se liste under)
- Valg av stasjon er irrelevant, men mange bruker `Q:` for `\\hn.helsenord.no\RHF\SKDE`.
...
*SAS*-filer må ha tegnsett `windows1252`. For å tvinge *vscode* til å åpne *SAS*-filer med dette tegnsettet kan følgende snutt legges inn i `settings.json` (man kan åpne denne filen ved å gå inn på *File/Preferences/Settings/*, velge *Text Editor* og *Files*. Der vil man få opp en link til *Edit in settings.json*):
